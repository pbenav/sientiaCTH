<div>
    ...
    <x-jet-dialog-modal wire:model="confirmingWorkCenterCreation">
        ...
        <x-slot name="content">
            ...
            <!-- NFC Tag Generation Section for Creation -->
            <div class="col-span-6 sm:col-span-4 mt-6">
                <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                    <div class="flex items-center justify-between mb-3">
                        <h4 class="text-sm font-medium text-green-900">{{ __('NFC Tag Content Preview') }}</h4>
                        <button 
                            type="button"
                    ...
                    <p class="text-xs text-green-700 mb-4">{{ __('Preview the content that will be programmed into the NFC tag (max 128 bytes)') }}</p>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center justify-between">
                                    <label class="text-xs font-medium text-green-800">{{ __('NFC Payload Preview') }}:</label>
                                    <button 
                                        type="button"
    ...
    <x-jet-dialog-modal wire:model="confirmingWorkCenterUpdate">
        ...
        <x-slot name="content">
            ...
            <div class="col-span-6 sm:col-span-4 mt-6">
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center">
                            <input id="edit_enable_nfc" type="checkbox" class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" wire:model.defer="state.enable_nfc">
                            <label for="edit_enable_nfc" class="ml-2 block text-sm text-gray-900">
                                {{ __('Enable NFC verification for this work center') }}
                            </label>
                        </div>
            ...
            <!-- NFC Tag Generation Section -->
            <div class="col-span-6 sm:col-span-4 mt-6">
                <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                    <div class="flex items-center justify-between mb-3">
                        <h4 class="text-sm font-medium text-green-900">{{ __('NFC Tag Content Generation') }}</h4>
                        <button 
                            type="button"
                    ...
                    <p class="text-xs text-green-700 mb-4">{{ __('Content to program into the physical NFC tag (automatically generated, max 128 bytes)') }}</p>
                    <div class="space-y-3">
                        @if(!empty($currentNFCContent))
                            <!-- Size Check -->
                            <div class="flex items-center justify-between p-2 rounded {{ ($currentNFCContent['size_bytes'] ?? 0) <= 128 ? 'bg-white border border-green-300' : 'bg-red-50 border border-red-300' }}">
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center justify-between">
                                    <label class="text-xs font-medium text-green-800">{{ __('Complete NFC Payload') }}:</label>
                                    <button 
                                        type="button"
                                        onclick="copyToClipboard('{{ $currentNFCContent['payload'] ?? '' }}')"
                                        class="px-2 py-1 text-xs text-gray-700 bg-white hover:bg-gray-50 rounded border border-gray-300 focus:outline-none">
                            ...
                            <!-- Status indicator -->
                            @if(!($currentNFCContent['is_existing'] ?? false))
                                <div class="bg-amber-50 border border-amber-300 rounded-lg p-2">
                                    <div class="flex items-center">
                                        <svg class="w-3 h-3 text-amber-600 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                                        </svg>
                                        <span class="text-xs text-amber-800">{{ __('Preview mode - Enable NFC above to save these settings') }}</span>
                                    </div>
                                </div>
                            @endif
        ...
        <x-slot name="footer">
            <x-jet-secondary-button wire:click="$set('confirmingWorkCenterUpdate', false)" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-jet-secondary-button>
