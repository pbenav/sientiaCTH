        ...
        <x-jet-dialog-modal wire:model="showModal" maxWidth="2xl">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-base font-bold text-gray-900">
                                {{ __('Event Details') }} <span class="text-gray-400 font-normal text-sm">#{{ $eventData['id'] }}</span>
                            </h3>
                            <p class="text-xs text-gray-500">{{ $eventData['user']['name'] ?? '' }} {{ $eventData['user']['family_name1'] ?? '' }}</p>
                        </div>
                    </div>
            ...
            <x-slot name="content">
                ...
                <div class="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-1.5">
                            <span class="w-2.5 h-2.5 rounded-full flex-shrink-0" style="background-color: {{ $eventData['event_type']['color'] ?? '#3788d8' }}"></span>
                            <span class="font-medium text-gray-950 truncate">{{ $eventData['event_type']['name'] ?? __('N/A') }}</span>
                        </div>
                    </div>
                    @endif
                ...
                <div class="flex items-center gap-4 text-xs pt-4 border-t border-gray-200 mt-4">
                    ...
                    <!-- GPS -->
                    @php
                        $hasGPS = isset($eventData['latitude']) && $eventData['latitude'] && isset($eventData['longitude']) && $eventData['longitude'];
                    @endphp
                    @if($hasGPS)
                    ...
                    <span class="flex items-center gap-1 {{ $hasIP ? 'text-purple-600' : 'text-gray-300' }}"
                        ...
                        <span class="{{ $hasIP ? 'font-medium font-mono' : '' }}">{{ $hasIP ? $eventData['ip_address'] : 'IP' }}</span>
                    </span>
                </div>
                    ...
                    <div class="mt-4 rounded-md bg-yellow-50 p-3 border border-yellow-100">
                        <div class="flex">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-xs text-yellow-700">{{ __('This event is closed and cannot be modified.') }}</p>
                            </div>
                        </div>
                    </div>
                @endif
                ...
                <div class="flex items-center justify-between w-full">
                    <div>
                        @if(!$eventData['is_open'] && $eventData['user_id'] == auth()->id())
                            <x-jet-button 
                                onclick="Livewire.emit('openReopeningModal', {{ $eventData['id'] }})"
